...
여러 에이전트가 도는 동안 무엇이 어디까지 갔는지 화면에서 보인다.
...
phase-02 가 흐름 하나를 만들었다. 실행 넷을 남기고 그중 둘이 나란히 돈다.
...
그 흐름은 오래 걸린다.
실측한 포지션 추천 하나가 입력 346만 토큰에 15분 넘게 걸렸고,
넷으로 나누면 더 걸릴 수도 있다.
...
누르면 `/executions/{executionId}` 로 간다.

흐름으로 돈 답에만 보인다. 실행 하나로 끝난 답에는 보이지 않는다.
plan010 의 phase-02 가 더한 `hasChildren` 으로 구분한다.
...
| `web/src/components/chat/` 의 진행 표시 부품 | 신규 또는 수정 |
