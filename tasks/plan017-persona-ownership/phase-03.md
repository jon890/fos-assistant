...
대시보드에 `SOUL.md` 를 읽는 경로가 없어 자동으로 옮길 수 없다.

**주인이 화면에서 한 번 붙여 넣는다.** 붙여 넣기 전까지 그 에이전트는 `NONE` 이고
홈서버 파일의 성격으로 그대로 돈다.
옮길 본문을 어디서 꺼내는지는 `fos-home-infra` 가 갖는다.

옮길 profile 이 하나도 남지 않으면 그 저장소에 알린다.
