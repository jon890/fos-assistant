...
| 순서 | 담는 것 |
| --- | --- |
| 1 | 가족 공용 Memory 중 `ACCEPTED` 인 것 |
| 2 | 요청자 개인 Memory 중 `ACCEPTED` 인 것 |

**다른 구성원의 개인 Memory 는 고르는 단계에서 빠진다.**
`MemoryService.injectableFor(user)` 가 그것을 보장한다.
문자열을 만든 뒤에 지우는 방식을 쓰지 않는다.

조립한 모양은 아래와 같다. 어느 것이 가족 공용이고 어느 것이 개인인지 에이전트가 알아야 한다.
