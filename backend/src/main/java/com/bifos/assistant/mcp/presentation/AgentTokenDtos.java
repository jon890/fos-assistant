...
public final class AgentTokenDtos {
    ...
    public record IssuedTokenResponse(Long id, String userEmail, String label, Instant createdAt, String token) {
        static IssuedTokenResponse from(IssuedToken issued) { return new IssuedTokenResponse(issued.token().id(), issued.userEmail(), issued.token().label(), issued.token().createdAt(), issued.rawToken()); }
    }
    public record TokenResponse(Long id, String userEmail, String label, Instant createdAt, Instant lastUsedAt, Instant revokedAt) {
        static TokenResponse from(TokenWithUser value) { var token = value.token(); return new TokenResponse(token.id(), value.userEmail(), token.label(), token.createdAt(), token.lastUsedAt(), token.revokedAt()); }
    }
}
