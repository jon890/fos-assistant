...
에이전트의 성격은 Control Plane 이 갖고 Hermes 의 `SOUL.md` 는 그 사본이다.
...
sequenceDiagram
    ...
    U->>W: 고친 본문과 지금 보고 있던 판 번호
    W->>C: PUT /api/v1/agents/{code}/persona
    C->>C: 고칠 수 있는 사람인지 본다
    C->>C: 판 번호가 지금 판과 같은지 본다
    C->>C: 다음 판으로 행을 만든다
...
**밀지 못해도 행은 남는다.**
그때는 반영 시각이 비어 있고 화면이 「아직 반영되지 않음」 과 다시 반영하는 단추를 보인다.

### 실행이 쓰는 판

실행은 데이터베이스를 거치지 않는다. Hermes 가 자기 `SOUL.md` 를 읽는다.
그래서 실행이 쓰는 것은 마지막 판이 아니라 **마지막으로 반영된 판**이다.
실행을 시작할 때 그 판의 번호를 실행 줄에 적는다.

### 갈리는 지점

| 무엇 | 어떻게 되나 |
| --- | --- |
| 아직 한 판도 없다 | 편집창이 비어 있고 「홈서버 파일의 성격으로 돌고 있다」 를 보인다 |
| 고칠 권한이 없다 | 본문을 읽기만 한다. 저장 단추가 없고 경로도 거절한다 |
| 볼 권한이 없다 | 그 에이전트가 목록에 없다. 본문 경로도 없는 에이전트와 같은 응답을 준다 |
| 본문이 비어 있다 | 거절한다. 빈 `SOUL.md` 를 밀면 성격이 지워진다 |
| 본문이 상한을 넘는다 | 거절한다. 화면이 저장 전에 남은 글자 수를 보인다 |
| 보고 있던 판이 지금 판이 아니다 | 거절한다. 그 사이 다른 사람이 고친 것이고, 화면이 새 본문을 다시 읽어 보인다 |
| 행은 만들었는데 밀지 못했다 | 행을 되돌리지 않는다. 미반영으로 두고 화면에 드러낸다 |
| 미반영 상태에서 대화가 들어온다 | 그대로 실행한다. 마지막으로 반영된 판으로 돌고 그 판을 실행 줄에 적는다 |
| 다시 반영을 눌렀는데 또 실패한다 | 미반영으로 그대로 남는다. 판을 새로 만들지 않는다 |
| 옛 판으로 되돌린다 | 그 본문으로 새 판을 만든다. 행을 지우거나 되살리지 않는다 |
