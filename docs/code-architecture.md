...
| 경로 | 하는 일 |
| --- | --- |
| `GET /api/v1/agents/{code}/persona` | 지금 본문과 고칠 수 있는지 |
| `PUT /api/v1/agents/{code}/persona` | 본문을 쓴다. 화면이 받아 간 본문을 함께 받는다 |

**화면이 받아 간 본문을 함께 받는 이유는 두 사람이 같은 에이전트를 고칠 수 있기 때문이다.**
`SOUL.md` 에는 판 번호가 없어서 값으로 달라진 것을 알 수 없다.
쓰기 직전에 다시 읽어 그것과 다르면 거절하고, 화면이 새 본문을 다시 읽는다.
