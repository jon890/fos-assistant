## ADR-002: 구성원마다 profile 을 나누되 AI 계정은 가족이 함께 쓴다
...
가족 구성원마다 자기 AI credential 을 연결할 수 있는 구조를 두려 했다.
...
구성원마다 계정을 나누려면 사람 수만큼 구독을 사야 한다.
가족 규모에서 그 비용은 얻는 것에 견줘 크다.

### 결정

**AI 계정은 가족이 함께 쓴다.**
구독 하나로 모든 구성원이 돌고, 누가 얼마나 썼는지는 계정이 아니라 실행 기록으로 구분한다.

profile 은 그래도 구성원마다 하나씩 둔다.
...
즉 한 구성원의 대화 내용이 다른 구성원의 profile 로 새지 않는다.
...
나머지 구성원은 그대로 공유를 쓴다. 둘이 한 시스템에 함께 있을 수 있다.

### 거절한 대안

- 구성원마다 구독을 따로 사는 방식은 가족 규모에서 비용이 얻는 것보다 크므로 지금은 쓰지 않는다.
...
- 계정 수준에서 보면 모든 대화가 한 사람 명의다. 제공자 쪽 기록은 구성원을 구분하지 않는다.
