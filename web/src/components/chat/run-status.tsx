...
export function RunStatus({ waiting, toolEvents }: Props) {
  ...
  return (
    <>
      {latestTool ? (
        <li
          className="grid grid-cols-[2rem_minmax(0,1fr)] gap-2 text-xs text-muted"
          aria-live="polite"
        >
          <span aria-hidden="true" />
          <span className="truncate">{latestTool}</span>
        </li>
      ) : null}
      {waiting ? (
        <li
        ...
        >
          ...
          <span
            className="flex min-h-8 items-center gap-1 motion-reduce:hidden"
            aria-hidden="true"
          >
          ...
          <span className="hidden min-h-8 items-center motion-reduce:flex">
            비서가 답을 준비하고 있다.
          </span>
        </li>
      ) : null}
